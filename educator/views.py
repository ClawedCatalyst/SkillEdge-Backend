    ...
    def put(self, request):
        ...
        if educator.is_verified == True:
            if educator.is_educator == False:
                ...
                return Response(
                    {"msg": "educator is already a educator"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"msg": "educator is not verified"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if educator.is_verified == True:
            if educator.is_educator == True:
                ...
                if avg_weighted_rating >= 2.5:
                    ...
                    educator.save()
                    return Response(
                        {"msg": "You are a certified educator"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                educator.is_certified_educator = False
                educator.save()
                return Response(
                    {"msg": "You are not a certified educator"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            return Response(
                {"msg": "user is not a educator"}, status=status.HTTP_400_BAD_REQUEST
            )
        return Response(
            {"msg": "user is not verified"}, status=status.HTTP_400_BAD_REQUEST
        )
