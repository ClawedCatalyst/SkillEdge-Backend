...
class BuyCourseView(APIView):
    ...
    def put(self, request, ck):
        ...
        if student.is_verified == False:
            return Response(
                {"msg": "You are not verified! Verify your mail"},
        ...
        if student.wallet < price:
            ...
            return Response(
                {"msg": "Low Balance!", "Amount to be added:": amount},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if serializer_student.is_valid():
            ...
            return Response(
                {"msg": "Your IN! Order Confirmation"}, status=status.HTTP_200_OK
            )
        return Response(
            {"msg": "transaction failed"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class BuyAllCourseView(APIView):
    ...
    def put(self, request):
        ...
        if len(courses) == 0:
            return Response(
                {"msg": "no courses in cart"}, status=status.HTTP_400_BAD_REQUEST
            )
