...
class Login_user(APIView):
    def post(self, request):
        ...
        if user is not None:
            ...
            return Response(
                {"id": user.id, "token": token, "msg": "Login Success"},
                status=status.HTTP_200_OK,
            )

        return Response(
            {"msg": "Enter correct Password"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                ...
                return Response(
                    {"token": token, "msg": "verification Successfull"},
                    status=status.HTTP_200_OK,
                )
